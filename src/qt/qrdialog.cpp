    ...
    model(0)
{
    ...
    GUIUtil::setFont({ui->labelQRCodeTitle}, GUIUtil::Weight::Bold, 16);
...
void QRDialog::update()
{
    ...
    if(!strQRCode.isEmpty())
    {
        ...
        for (int y = 0; y < code->width; y++)
        {
            ...
            {
                myImage.setPixel(x + 4, y + 4, ((*p & 1) ? GUIUtil::getThemedQColor(GUIUtil::ThemedColor::QR_PIXEL).rgb() :
                                                           GUIUtil::getThemedQColor(GUIUtil::ThemedColor::BACKGROUND_WIDGET).rgb()));
                p++;
            }
        }
        QRcode_free(code);
