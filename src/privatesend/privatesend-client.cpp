...
bool CPrivateSendClientSession::DoAutomaticDenominating(CConnman& connman, bool fDryRun)
{
...

    {
        ...
        if (nBalanceAnonimizableNonDenom >= nValueMin + CPrivateSend::GetMinCollateralAmount() && nBalanceToDenominate > 0) {
...
bool CPrivateSendClientSession::MakeCollateralAmounts(const CompactTallyItem& tallyItem, bool fTryDenominated)
{
    ...
    if (!txBuilder.CouldAddOutput(CPrivateSend::GetMinCollateralAmount())) {
        return false;
    }

    int nCase{0}; // Just for debug logs
    if (txBuilder.CouldAddOutputs({CPrivateSend::GetMaxCollateralAmount(), CPrivateSend::GetMinCollateralAmount()})) {
        nCase = 1;
        // <case1>, see TransactionRecord::decomposeTransaction
        // Out1 == CPrivateSend::GetMaxCollateralAmount()
        // Out2 >= CPrivateSend::GetMinCollateralAmount()
    ...
    } else if (txBuilder.CouldAddOutputs({CPrivateSend::GetMinCollateralAmount(), CPrivateSend::GetMinCollateralAmount()})) {
        nCase = 2;
    ...
    } else { // still at least possible to add one CPrivateSend::GetMinCollateralAmount() output
        nCase = 3;
