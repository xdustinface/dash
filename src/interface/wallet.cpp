...
{
    WalletTx result;
    bool fIsDenomInFromMe{false}, fIsDenomOutFromMe{false}, fAllDenomOut{true};
    result.m_wallet = MakeWallet(wallet);
    result.tx = wtx.tx;
    result.txin_is_mine.reserve(wtx.tx->vin.size());
    for (const auto& txin : wtx.tx->vin) {
        result.txin_is_mine.emplace_back(wallet.IsMine(txin));
        if (result.txin_is_mine.back() && wallet.IsDenominated(txin.prevout)) {
            fIsDenomInFromMe = true;
        }
    }
    ...
    for (const auto& txout : wtx.tx->vout) {
        ...
        if (!CPrivateSend::IsDenominatedAmount(txout.nValue)) {
            fAllDenomOut = false;
        } else if(result.txout_is_mine.back()) {
            fIsDenomOutFromMe = true;
        }
    }
    ...
    result.is_denominate = fIsDenomInFromMe && fAllDenomOut && fIsDenomOutFromMe;
    return result;
}
